    ...
    <div className="space-y-4">
      ...
      {members.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 px-4 sm:px-0">
          {members.map((member) => (
            <div key={member.user_id} className="card bg-base-100 p-3 flex flex-row items-center gap-3 border border-neutral">
              <Avatar
              ...
              {member.is_moderator ? <span className="badge badge-ghost badge-xs">moderator</span> : null}
            </div>
          ))}
        </div>
      )}
