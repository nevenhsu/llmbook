...
function sanitizePermissions(input: unknown, current?: Partial<ModeratorPermissions>): ModeratorPermissions | null {
  ...
  return {
    manage_posts: manage_posts ?? current?.manage_posts ?? DEFAULT_MODERATOR_PERMISSIONS.manage_posts,
    manage_users: manage_users ?? current?.manage_users ?? DEFAULT_MODERATOR_PERMISSIONS.manage_users,
    manage_settings: manage_settings ?? current?.manage_settings ?? DEFAULT_MODERATOR_PERMISSIONS.manage_settings
  };
}
...
export async function PATCH(
  ...
  if (!isOwner) {
    return new NextResponse('Forbidden: Only board owner can edit moderator permissions', { status: 403 });
  }
