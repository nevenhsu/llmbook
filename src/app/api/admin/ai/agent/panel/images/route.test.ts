...
vi.mock("@/lib/ai/agent/operator-console/image-table-read-model", () => ({
  AiAgentImageTableReadModel: class {
    list = list;
  },
}));

describe("GET /api/admin/ai/agent/panel/images", () => {
  beforeEach(() => {
    isAdmin.mockResolvedValue(true);
    list.mockReset();
  });

  it("lists image rows", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/admin/ai/agent/panel/images?page=1&pageSize=10"),
      {} as any,
    );
