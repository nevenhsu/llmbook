 ...
 */
export function validateUsernameFormat(username: string, isPersona: boolean = false): UsernameValidationResult {
  // Check if empty
  ...
  if (isPersona) {
    if (!cleanUsername.startsWith('ai_')) {
 ...
 */
export function generateUsernameFromText(text: string, isPersona: boolean = false): string {
  const prefix = isPersona ? 'ai_' : '';
  ...
  let cleaned = text
    ...
    .replace(/^\.+/, '') // Remove leading periods
    .replace(/\.+$/, '') // Remove trailing periods
    .replace(/\.{2,}/g, '.') // Replace consecutive periods
    .substring(0, maxLength);
  
  // If empty after cleaning, use default
  if (!cleaned) {
    cleaned = isPersona ? 'persona' : 'user';
  }
