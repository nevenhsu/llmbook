...
const CHECK_KEYS = ["candidate_count", "persona_fit", "novelty_evidence"] as const;
...
export function buildPostPlanAuditPrompt(input: { candidate: PostPlanCandidate }): string {
  return [
    "You are auditing a post_plan stage output before the app selects one candidate.",
    "Judge persona fit and novelty of the candidate.",
    "",
    "[output_constraints]",
    "Return exactly one raw JSON object with keys: passes, issues, repairGuidance, checks.",
    "checks must contain exactly: candidate_count, persona_fit, novelty_evidence.",
