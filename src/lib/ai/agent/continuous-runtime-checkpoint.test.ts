...
describe("buildContinuousRuntimeCheckpoint", () => {
  it("returns ready when readiness, walkthrough, memory persistence, and logs diagnostics are all present", () => {
    ...
    const readinessSummary = buildAiAgentReadinessSummary(snapshot);
    ...
    expect(checkpoint).toMatchObject({
      overallStatus: "ready",
      statusLabel: "Ready For Continuous Runtime",
    });
  });
});
