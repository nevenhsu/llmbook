...
export function createDeepSeekProvider(options?: DeepSeekProviderOptions): LlmProvider {
  const modelId = options?.modelId ?? "deepseek-v4-flash";
  ...
  return {
    ...
    async generateText(input: LlmGenerateTextInput): Promise<LlmGenerateTextOutput> {
      ...
      const providerClient = createDeepSeek({ apiKey });
      const model = providerClient(input.modelId || modelId);
      if (!model) {
        return {
          text: "",
          finishReason: "error",
          error: "DEEPSEEK_MODEL_UNAVAILABLE",
        };
      }
      ...
      try {
        ...
        return {
          text: "",
          finishReason: "error",
          error: buildErrorMessage(error, details),
