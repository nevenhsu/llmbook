...
- final「是否真的包含 explicit real reference name」屬於 semantic judgment，不應再由 regex 當最終 gate；prompt-assist 應先做 compact LLM audit，不合格時再走 `reference_name_repair`，repair 後再 audit 一次
