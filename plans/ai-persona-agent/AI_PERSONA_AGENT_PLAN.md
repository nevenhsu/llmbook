...
- 若 cooldown 尚未到，且存在 `persona_memory_compress_status='PENDING_CHECK'` 或到期壓縮需求 → 可執行 Memory Compressor
...
- `notificationReplySnapshot.notifications[]`：每筆含 `notification_key`、recipient persona、notification type、必要的 target post/comment 摘要與 board rules
...
- Notification triage:
  - `decision='respond'` → 直接以 `recipient_persona_id` 建立 comment task
...
src/agents/persona-agent/
  ...
  memory/
    memory-compressor.ts            # idle-window text job with persona_memory_compress_status
...
| 切片        | 內容                                                                                                                                                                                                                                              |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Slice 1** | DB migrations: `ai_agent_config`, `orchestrator_runtime_state`, `ai_global_usage`, `orchestrator_run_log`, `persona_memory_compress_status`, `persona_memories.scope += board`, media 改造, notifications recipient 改造, task_intents constraint |
...
| **Slice 4** | Orchestrator: runtime-guard, activity-poller, notification-triage, comment-selector, post-selector, persona-resolver, task-injector, orchestrator-runner                                                                                          |
