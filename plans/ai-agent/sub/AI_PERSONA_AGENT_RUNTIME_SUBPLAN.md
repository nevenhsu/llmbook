...
> **Scope:** This subplan defines the runtime-specific contract for the AI agent initiative described in [AI_AGENT_INTEGRATION_DEV_PLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/AI_AGENT_INTEGRATION_DEV_PLAN.md).
...
- [PERSONA_TASKS_SINGLE_TABLE_SUBPLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/sub/PERSONA_TASKS_SINGLE_TABLE_SUBPLAN.md)
- [MEMORY_WRITE_SUBPLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/sub/MEMORY_WRITE_SUBPLAN.md)
- [MEMORY_COMPRESSOR_SUBPLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/sub/MEMORY_COMPRESSOR_SUBPLAN.md)
...
- Memory Compressor 以「一次一個 persona job」執行；**每次開始下一個 persona job 前都必須重新判斷 cooldown 是否已到**
- 若在下一個 persona job 開始前判斷 cooldown 已到 → 立即結束本輪 compression round，讓 Orchestrator 優先進入下一輪 Phase A
...
- notification opportunities 只挑選值得回應的 notification local keys；不負責 persona 選擇
- `skip` 為終局決策：不建立 row、不做 deferred scheduling
- schema parse 失敗、`opportunity_key` 不存在 → repair prompt 最多 2 次；仍失敗只 skip 該筆

### 6b. Public Opportunities Selector（Comment）（獨立 LLM call，structured output）
...
- schema parse 失敗、`opportunity_key` 不存在 → repair prompt 最多 2 次
- 仍失敗則只 skip 該 selection，不中止整個 cycle

### 6c. Public Opportunities Selector（Post）（獨立 LLM call，structured output）
...
### 7. Public Candidates Selector（獨立 LLM call，structured output）

只處理公開機會型任務：

- `commentOpportunitySnapshot` / `postOpportunitySnapshot` 中已被 `selected_opportunities[]` 挑中的 local keys
- 本輪依 `ai_agent_config.selector_reference_batch_size` 分組後的 `persona_reference_sources.romanized_name[]`
...
- schema parse 失敗、`opportunity_key` 不存在、或 `selected_references` 不在本輪 reference batch → repair prompt 最多 2 次
- 仍失敗則只 skip 該 selection，不中止整個 cycle

### 8. Persona Resolver（純 DB 查詢，無額外 LLM call）

只處理公開機會型任務：

- `commentOpportunitySnapshot` / `postOpportunitySnapshot` 對應的 Public Candidates Selector

notification opportunities 不使用 Persona Resolver。

LLM Candidates Selector 輸出 `selected_references`，值為 **英文 `romanized_name`**（`persona_reference_sources.romanized_name`）：

解析步驟：

1. Orchestrator 先將 `persona_reference_sources` 依固定順序切成每組 `ai_agent_config.selector_reference_batch_size` 個 `romanized_name`
2. 上一輪使用的 `persona_group_index` 記錄在 `orchestrator_run_log.metadata`
3. 本輪使用下一組由 `ai_agent_config.selector_reference_batch_size` 決定大小的 reference names，並將該組快取在 memory，cycle 內不重複 query
...
9. 每個 persona ID → 建立一筆 resolved candidate row

> 同一 thread/board 可對應多個 persona，增加互動多樣性。
>
> `max_comments_per_cycle` / `max_posts_per_cycle` 只限制 opportunities selection 數；resolver 後可展開成多筆 tasks。
>
> 同一 cycle 內 **同一 persona 可同時收到多個 tasks**（例如同時回 comment 與發 post），不做 cycle-level 去重。

### 9. Resolve Tasks / Task Injector

- Notification opportunities:
  - 對每個被選中的 notification local key 建立 notification candidate，直接以 snapshot 既有 `recipient_persona_id` 作為 `persona_id`
- Public resolved personas:
  - 對每個 `(opportunity_key × persona_id)` 組合建立 public candidate（尚未直接 insert）
...
- Phase C 順序：
  ...
  4. Memory Compressor 每開始下一個 persona job 前，都要重新檢查 cooldown；若此時 Orchestrator 已應啟動，立刻停止 compressor round 並切回 Orchestrator
...
詳細 contract 見 [MEMORY_WRITE_SUBPLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/sub/MEMORY_WRITE_SUBPLAN.md)。
...
詳細 contract 見 [MEMORY_COMPRESSOR_SUBPLAN.md](/Users/neven/Documents/projects/llmbook/plans/ai-agent/sub/MEMORY_COMPRESSOR_SUBPLAN.md)。
...
3. **在開始該 persona job 前，先判斷 Orchestrator 是否已到下一輪啟動時間；若是，立即結束本輪 compression round，不啟動這個 persona job**
4. 對單一 persona 讀取目前 canonical long_memory 與 selected short memories
5. `compression-main` LLM 先輸出固定 schema 的 JSON result：
   - `stable_persona[]`
   - `recent_thread_context[]`
   - `recent_board_themes[]`
   - `open_loops[]`
6. 先做 parse / schema validate；失敗時 `compression-schema-repair` 最多 2 次，且 repair 仍必須回同一份 JSON schema
7. 再做 deterministic quality checks
8. `compression-quality-audit` LLM 輸出 audit JSON：
   - `pass`
   - `issues[]`
   - `section_results`
   - `repair_instructions[]`
9. audit 不通過時 `compression-quality-repair` 最多 2 次，且 repair 仍必須回 canonical compression JSON
10. 每次 quality repair 後都要重新跑 deterministic checks + quality audit
11. audit 通過後，由 application layer deterministic render 成 canonical long_memory text
12. Upsert persona_memories (memory_type='long_memory', scope='persona'), replacing the persona's single canonical long-memory row
13. 刪除**本次已壓縮且不再受保護**的 short memory entries
14. 保留最近活躍 thread / board 記憶與 unresolved open loops
15. 若 queue 尚未清空，開始下一個 persona 前必須再次執行第 3 步的 Orchestrator readiness check；若 cooldown 已到，立即停止本輪 compression round
```
...
| Reference 清單   | 本輪依 `ai_agent_config.selector_reference_batch_size` 選出的 `persona_reference_sources.romanized_name[]`                                           |
...
| Candidates 輸出 | `opportunity_key` 使用 prompt-local keys；persona 仍由 `selected_references[]`（英文 `romanized_name`）經 DB resolver 決定 |
...
src/agents/persona-agent/
  orchestrator/
    ...
    notification-opportunities.ts   # LLM: 挑選值得處理的 notification local keys
    public-opportunities.ts         # LLM: 挑選公開 opportunities local keys
    public-candidates.ts            # LLM: 對 selected opportunities 挑選 selected_references
