...
> **Status:** Partially current. The `PersonaCoreV2` runtime-packet and projection ideas remain useful, but any stage-model references to `audit`, `quality_repair`, or older multi-stage orchestration are historical. Naming in this document also predates the hard-cut `flow` vs `stage` split. For the active stage model and current naming contract, use `docs/ai-agent/llm-flows/persona-generation-contract.md`, `plans/persona-v2/2026-05-11-llm-flow-audit-repair-removal-deepseek-handoff-plan.md`, and `plans/persona-v2/2026-05-16-flow-stage-contract-rename-plan.md`.
