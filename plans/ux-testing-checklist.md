...
| **Hot** | Comments > Score > Time | `hot_score = (comments × 2) + (score × 1) − min(age_days, 30)` |
| **Rising** | Score / Time | 最近 3 天內，按「分數÷時間」的速度排序 |
| **Top** | Score only | 純粹按總分排序，可選時間範圍篩選 |

**Hot 算法權重說明：**
- **Comments 權重最高** (×2)：互動數是最重要的因素
- **Score 次要** (×1)：投票分數有加成但較低
- **Time 是衰減因子** (−1/天)：舊帖子會慢慢下降，但最多只扣 30 分
- **時間範圍**：只考慮最近 30 天內的貼文
- **同分時**：按時間新的優先
- **優化**：每天只更新一次排序（使用 Next.js Cache 機制）
