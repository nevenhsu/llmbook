...
- **Sibling Flow Builders:** When the user wants sibling flows like `comment` and `reply` to follow the `post` canonical prompt-builder pattern, mirror the current `post` structure literally: separate `src/lib/ai/prompt-runtime/<flow>/` folders, same block order shape as post, and flow-owned prompt-visible builders instead of a combined sibling module.
- **Preview Removal Scope:** When the user says to remove preview features for flows, do not preserve admin preview seams out of habit. First map where preview exists, then plan deletion of flow preview entrypoints, routes, mocks, and tests unless the user explicitly keeps a debug-only surface.
- **Preview Terminology:** In this repo, "preview mock" and the real admin manual test feature are different things. Do not collapse them. If the user says remove preview mocks, delete only mock pages, fixtures, and mock helpers while preserving the live admin preview route/service unless they explicitly say otherwise.
- **Flow Module Shape:** If the user says sibling flows should look structurally closer to `post-flow-module.ts`, do not hide their orchestration inside a shared executor like `single-stage-writer-flow.ts`. Let each flow module absorb its own orchestration and keep sharing only narrow mechanical helpers if they remain clearly non-owning.
- **Prompt Debug Surface:** If stage debug records already carry the prompt/debug surface, do not keep or reintroduce a parallel API field for preview prompt text just because older preview contracts had `assembledPrompt`. Remove the redundant prompt-return field from the active flow API contract unless the user explicitly wants both.
- **Shared Preview Contracts:** If the user says all preview flows should rely on debug records instead of returned prompt text, do not special-case persona generation or preserve a shared `assembledPrompt` field for convenience. Split or shrink the active preview contracts so prompt inspection has one canonical surface: `stageDebugRecords`.
- **Local Prompt Inspection:** If the user still wants a prompt-inspection UI after removing API-returned prompt text, do not put `assembledPrompt` back into preview responses. Build the prompt locally from the canonical `src/lib/ai/prompt-runtime/*` functions with admin/mock inputs and keep that path separate from the live preview API contract.
- **Prompt Test Source:** If tests need prompt assertions, do not keep preview-response prompt fields just for test convenience. Tests should call the canonical prompt-runtime helpers directly and assert on that local builder output instead of the API payload.
- **Delete Temporary Preview Helpers:** If a helper like `execution-preview.ts` survives only by being refactored off a mock dependency, do not keep it as a renamed compromise. Move the useful deterministic logic into the real owners, then delete the helper completely if the user asks for that end state.
- **Archive Scope:** Do not touch archived docs just because they mention retired shapes. Update only active/non-archived docs unless the user explicitly asks to revise archive material too.
...
- **Structural Prompt Refactors:** When the user says they will update prompt text later, keep new flow-owned prompt modules structurally correct but leave prompt copy as placeholder/current wording. Do not start rewriting `comment` / `reply` prompt text just because the structural refactor is complete.
