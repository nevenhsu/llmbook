...
- In staged preview generation, a parseable quality-repair result is not automatically "done"—if the repaired JSON still violates deterministic quality rules like English-only or mixed-script checks, feed those remaining issues into another quality-repair round instead of failing immediately.
- In staged preview generation, a non-empty but malformed quality-repair response should get one more strict JSON-only retry before surfacing `quality repair returned invalid JSON`; otherwise a single half-broken repair attempt ends the stage too early.
- When generated reference data mixes people with concepts or works, split the contract explicitly (`reference_sources` vs `other_reference_sources`) and let LLM audit decide which references remain in the personality-bearing bucket; do not hide the distinction only inside DB write-path filtering.
...
- When a stage-local shape drift is semantically identical, like `task_style_matrix.comment.body_shape` standing in for `feedback_shape`, normalize it in the parser instead of failing the whole preview.
